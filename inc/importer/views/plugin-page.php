...
?>
    ...
    <div class="ocdi  wrap  about-wrap">
        <div class="busify-title"><span class="dashicons dashicons-external"></span> Busify Demo Importer</div>
		<?php ob_start(); ?>
		<?php
		// Display warrning if PHP safe mode is enabled, since we wont be able to change the max_execution_time.
		if ( ini_get( 'safe_mode' ) ) {
			printf( esc_html__( '%sWarning: your server is using %sPHP safe mode%s. This means that you might experience server timeout errors.%s', 'pt-ocdi' ), '<div class="notice  notice-warning  is-dismissible"><p>', '<strong>', '</strong>', '</p></div>' );
		}
		...
		<?php elseif ( 1 === count( $predefined_themes ) ) : ?>
            <div class="busify-imported-logger"></div>

            <div class="busify-flexbox-container">
                <div class="busify-left-box">
                    <h3>Following data will be imported:</h3>
                    <ol class="busify-site-contents">
                        <li class="busify-sites-import-customizer">Customizer Settings</li>
                        <li class="busify-sites-import-content">Contents</li>
                        <li class="busify-sites-import-widgets">Widgets</li>
                        <li class="busify-sites-import-widgets">Menus</li>
                    </ol>

					<?php if ( PluginsInstaller::check_required_plugin_installed_activated() ) { ?>
                        <hr>
                        <p>Click below button to start import process. This will take some time depending on the server.</p>
                        <p class="ocdi__button-container">
                            <button class="ocdi__button  button  button-hero  button-primary  js-ocdi-import-data"><?php esc_html_e( 'Import Demo Data', 'pt-ocdi' ); ?></button>
                        </p>
					<?php } else { ?>
                        <div class="notice notice-warning settings-error">
                            <p>All required plugins are not installed or activated. Please install and activate them in order to import demo properly !</p>
                        </div>
					<?php } ?>
                </div>
                <div class="busify-right-box">
                    <img src="<?php echo CODEMANAS_THEME_URL . 'assets/images/busify-theme.png'; ?>">
                </div>
            </div>
