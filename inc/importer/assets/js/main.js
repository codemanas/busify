jQuery( function ( $ ) {
	...
	function ajaxCall( data ) {
		var importerBtn = $('.js-ocdi-import-data');
		var busifyImportedLogger = $('.busify-imported-logger');
		$.ajax({
			method:      'POST',
			url:         ocdi.ajax_url,
			data:        data,
			...
			beforeSend:  function() {
				...
				$(busifyImportedLogger).show().html("Importing Images and Contents..");
                $(importerBtn).hide();
			}
		})
		.done( function( response ) {
			if ( 'undefined' !== typeof response.status && 'newAJAX' === response.status ) {
				...
				$(busifyImportedLogger).show().html('Importing Customizer Settings..');
				...
				$(busifyImportedLogger).show().html('Importing theme settings Settings..');
				// Fix for data.set and data.delete, which they are not supported in some browsers.
				var newData = new FormData();
